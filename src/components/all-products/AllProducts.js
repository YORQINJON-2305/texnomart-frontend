        ...
        {products?.map((pr) => (
          <div key={pr._id} className="excelent_product">
             ...
             <div className="u_p_wrap">
                <Link  className='update_btn' style={{textDecoration: "none", color: "#fff", fontWeight: "550"}} to={`/update/${pr._id}`}>Update</Link> 
               <form onSubmit={deleteHandler}>
                   <button className='delete_btn' onClick={() => setId(pr._id)}>Delete</button>
               </form>
             </div>
         </div>
        ))}
